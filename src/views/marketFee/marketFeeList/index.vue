		...
		<div class="page-con" v-loading="loading" element-loading-text="加载中...">
			...
			<div class="table-data">
				<el-table :data="list" :emptyText="emptyText" border show-summary :summary-method="getSummaries" style="width:100%">
					...
					<el-table-column prop="cmd" label="买卖" label-class-name="labelClass" min-width="100">
						<template slot-scope="scope">
							<div v-if="scope.row.cmd == '0'">买入</div>
							<div v-if="scope.row.cmd == '1'">卖出</div>
							<div v-if="scope.row.cmd == '2'">递延</div>
							<div v-if="scope.row.cmd == '3'">盈利分成</div>
						</template>
					</el-table-column>
					<el-table-column prop="market_user" label="分成人" label-class-name="labelClass" min-width="100"></el-table-column>
					...
					<el-table-column prop="" label="操作" label-class-name="labelClass" min-width="100" fixed="right">
						<template slot-scope="scope">
							<span @click="$refs.obtainSublevel.policyDetails('market','详情',scope.row.id)" style="color:#20a0ff;cursor: pointer;">查看详情</span>
						</template>
					</el-table-column>
				</el-table>
			</div>
