import { commonRequest } from '@/api/api-defer'
...
export default {
	...
	methods: {
		numberFormat(a, b){
			console.log(a)
			// let num = Math.round(Number(cellValue)*100)/100
		  // return Number(num)
		},
	  getList(){
			const dataTimeObj = this.timeFormatFn(this.checkTime)   //获取全局时间处理方法
		  this.loading = true
		  commonRequest('delayfeeList',{
...
formattedData(){
	for(let i = 0;i<this.list.length;i++){
		this.list[i].left_volume = Number(this.list[i].left_volume) 
